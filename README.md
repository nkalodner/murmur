...
- **Enabling reports that Windows removed the startup entry**: some antivirus and "startup manager" tools revert startup changes made by apps they do not recognize. Murmur notices this (it reads the entry back), tries a Startup folder shortcut as a fallback, and tells you if that is blocked too. When it is, allow Murmur (`murmurw.exe`) in that tool, or add the shortcut yourself: press Win+R, run `shell:startup`, and drop a shortcut to `murmurw.exe` in the folder that opens.
