...
- **Swapping models can no longer leave you with nothing.** The new model downloads and loads in the background while the one you have keeps working, and a pick that fails (a mistyped repo id, a download that dies, an int8 build this CPU refuses) puts the old one straight back and says why on the Model tab. Before, a bad pick meant a Murmur that errored on every dictation with nothing to tell you, which is how people ended up reinstalling. The int8 case goes one better: Murmur retries at full precision by itself and tells you it did.
...
For strong Mandarin, `onnx-community/whisper-large-v3-turbo` at **full precision** is the pick. Its int8 build fails to load on most CPUs (`NOT_IMPLEMENTED ... ConvInteger`), which is not a broken download; since 0.14.0 Murmur notices and switches to full precision by itself.
